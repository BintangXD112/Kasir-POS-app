...
import { JenisProduk as JenisProdukType } from "@/types/type";
import Swal from 'sweetalert2';
import { router } from '@inertiajs/react';

export default function JenisProduk({ jenis_produk, currentTheme }: { jenis_produk: JenisProdukType[] }) {
  const [searchTerm, setSearchTerm] = useState('')
  const [showEditModal, setShowEditModal] = useState(false);
  const [editData, setEditData] = useState<JenisProdukType | null>(null);
  const [showAddModal, setShowAddModal] = useState(false);
  const [addData, setAddData] = useState({ nama_jenis_produk: '' });
  ...
  const openEditModal = (jenis_produk: JenisProdukType) => {
    setEditData({
      id: jenis_produk.id,
      nama_jenis_produk: jenis_produk.nama_jenis_produk ?? '',
      created_at: jenis_produk.created_at ?? null,
      updated_at: jenis_produk.updated_at ?? null,
    });
  ...
  const filteredJenisProduk = useMemo(()=>{
    if(!searchTerm) return jenis_produk;
    return jenis_produk.filter(item=>
      item.nama_jenis_produk.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [searchTerm, jenis_produk])
  ...
  const handleDelete = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.delete(route('jenis_produk.destroy', id), {
          onSuccess: () => {
            Swal.fire('Terhapus!', 'jenis produk berhasil dihapus.', 'success');
          },
          onError: () => {
            Swal.fire('Gagal!', 'Gagal menghapus jenis produk.', 'error');
          },
        });
      }
    });
  ...
  return (
    <div className={`p-6 max-w-full mx-auto min-h-[75vh] ${appBg} ${text} rounded-xl shadow text-black`}>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Data jenis produk</h2>
      </div>
      <div className={`flex justify-between items-center mb-6 ${card} shadow rounded-xl p-6`}>
        <input
          ...
          placeholder="Cari jenis jenis produk..."
        />
        <button
      ...
      <div className={`overflow-x-auto min-h-[40vh] ${card} relative pb-20 rounded-xl shadow`}>
        <table className="min-w-full text-sm text-left">
          <thead className={`${currentTheme === 'auto'? 'bg-slate-50 dark:bg-zinc-800' : currentTheme === 'Dark' ? 'bg-zinc-800' : 'bg-slate-50'}`}>
            <tr className="border-b border-gray-200">
              <th className="py-3 px-6">Nama Jenis Produk</th>
              <th className="py-3 px-6 text-center">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {filteredJenisProduk.length > 0 && pagedJenisProduk.map((item) => (
              <tr key={item.id} className={`border-b transition ${borderSoft} ${rowHover}`}>
                <td className="py-3 px-6">{item.nama_jenis_produk}</td>
        ...
        {filteredJenisProduk.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="mt-1 text-sm text-slate-500">Tidak ada Data jenis produk yang sesuai dengan filter yang dipilih.</p>
              </div>
            )}
        {/* pagination */}
        {filteredJenisProduk.length > 0 && (
          <div className={`flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t ${borderSoft} absolute bottom-0 left-0 right-0 gap-3`}>
            <div className={`text-sm ${subText}`}>
              ...
              <span className="font-semibold"> {totalItems}</span> Jenis Produk
            </div>
            ...
            <div className={`${card} ${text} p-6 rounded-lg w-full max-w-md`}>
              ...
              <form 
                onSubmit={(e) => {
                  ...
                  formData.append("nama_jenis_produk", editData.nama_jenis_produk);
                  formData.append('_method', 'PUT');

                  router.post(`/jenis_produk/${editData.id}`, formData, {
                    forceFormData: true,
                    onSuccess: () => {
                      closeModal();
              ...
              >
                ...
                <input
                  type="text"
                  value={editData?.nama_jenis_produk ?? ""}
                  onChange={(e) => setEditData({ ...editData, nama_jenis_produk: e.target.value })}
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50v backdrop-blur-sm">
          <div className={`${card} ${text} p-6 rounded-lg w-full max-w-md`}>
            <h2 className="text-xl font-semibold mb-4">Tambah jenis produk</h2>
            <form
              onSubmit={e => {
                e.preventDefault();
                const formData = new FormData();
                formData.append('nama_jenis_produk', addData.nama_jenis_produk);
                router.post('/jenis_produk', formData, {
                  forceFormData: true,
                  onSuccess: () => {
                    setShowAddModal(false);
                    setAddData({ nama_jenis_produk: '' });
            ...
            >
              ...
              <input
                type="text"
                value={addData.nama_jenis_produk}
                onChange={e => setAddData({ ...addData, nama_jenis_produk: e.target.value })}
                className="w-full border p-2 mb-2 border border-gray-400 rounded"
                placeholder="Nama jenis produk"
                required
              />
